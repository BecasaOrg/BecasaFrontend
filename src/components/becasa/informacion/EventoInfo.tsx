...
export default function EventoInfo({ fechaInicio, fechaFin, direccion, ciudad }: EventoInfoProps) {
  ...
  const cards: InfoCard[] = [
    ...
    },
    {
      ...
      titulo: (
        <>
          8:00<span className="align-baseline">a.m.</span>
          {" "}a 4:00<span className="align-baseline">p.m.</span>
        </>
      ),
    ...
    <div className="flex flex-col md:flex-row gap-4 justify-center items-center py-8">
      {cards.map((card, i) => {
        ...
        return (
          <div
            key={i}
            className="relative group flex flex-col justify-between w-[95%] md:w-[30%]  h-48 sm:h-42 md:h-48 rounded-2xl p-5 overflow-hidden bg-white border border-black/20 hover:bg-[#0A0E2A] "
          >

            <div
              className={`absolute w-[140vw] h-[10rem] md:w-[40vw] md:h-[8rem] ${card.style} group-hover:opacity-0 `}
              style={{
            ...
            <div className="flex items-center justify-between z-1">
              <span className="bg-[#0A0E2A] text-white text-xs font-light px-3 py-1 rounded-full tracking-wide group-hover:bg-white group-hover:text-[#0A0E2A] ">
                {card.label}
              </span>
            ...
            <div className="z-1">
              <div
                className="text-black group-hover:text-white  leading-none mb-1.5 text-[15px] lg:text-[20px] "
                style={{ letterSpacing: "-0.01em", transform: "scaleY(1.2)" }} >
                {card.titulo}
              </div>
